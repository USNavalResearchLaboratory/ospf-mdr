...
static void
ospf6_top_lsdb_hook_replace (struct ospf6_lsa *old, struct ospf6_lsa *new)
{
  assert (OSPF6_LSA_IS_SAME (old, new));

  ospf6_top_lsdb_hook_remove (old);
  ospf6_top_lsdb_hook_add (new);
}
...
{
  ospf6_abr_examin_brouter (ospf6_adv_router_in_prefix (&route->prefix));
...
ospf6_create (void)
{
  ...
  o->lsdb->hook_replace = ospf6_top_lsdb_hook_replace;
  ...
  o->min_lsa_interval = MIN_LS_INTERVAL;

  o->ipv6_tclass = OSPF6_DEFAULT_IPV6_TCLASS;
  ...
  if (CHECK_FLAG (o->flag, OSPF6_DISABLED))
    {
      ...
      ospf6_asbr_redistribute_enable (o);
    }
}
  ...
  if (! CHECK_FLAG (o->flag, OSPF6_DISABLED))
    {
      ...
      ospf6_asbr_external_table_finish (o->external_table);
      ospf6_route_remove_all (o->external_table);
      ...
      for (ALL_LIST_ELEMENTS_RO (oa->if_list, j, oi))
        {
	      ...
	      /* neighbor is in state Exchange or Loading; try again
		 later (database synchronization must complete) */
	      o->maxage_remover =
		thread_add_timer (master, ospf6_maxage_remover, o, 2);
              return 0;
            }
        }
    }
...
/**
 * Return a pseudo-random number between 0 and @a max, inclusive.
 */
unsigned int
ospf6_random (unsigned int max)
{
...
DEFUN (ospf6_instance_id,
       ospf6_instance_id_cmd,
       "instance-id <0-255>",
       ...
       "Instance ID value\n")
{
  ...
  enabled = !CHECK_FLAG (o->flag, OSPF6_DISABLED);
  if (enabled && o->instance_id != instance_id)
    {
      vty_out (vty, "Disabling current OSPFv3 Instance %u; "
	       "changing to new Instance ID %u%s",
	       o->instance_id, instance_id, VNL);
      ospf6_disable (o);
    }

  o->instance_id = instance_id;

  if (enabled && CHECK_FLAG (o->flag, OSPF6_DISABLED))
    ospf6_enable (o);
       ...
       V4NOTATION_STR)
{
  ...
  ret = ospf6_str2id (argv[0], &router_id);
  if (ret)
    {
      ...
      )
{
  ...
  /* parse Area-ID */
  if (ospf6_str2id (argv[1], &area_id))
    {
  ...
  /* Remove OSPF interface prefixes from redistribution */
  ospf6_asbr_redistribute_remove_connected (oi);
...
DEFUN (ospf6_min_lsa_arrival,
       ospf6_min_lsa_arrival_cmd,
       "min-lsa-arrival <0-65535>",
       "minimum time between receiving new LSA instances\n"
       "Seconds\n")
{
  struct ospf6 *o;
  ...
  o->min_lsa_arrival = strtol (argv[0], NULL, 10);

  return CMD_SUCCESS;
}

DEFUN (ospf6_min_lsa_interval,
       ospf6_min_lsa_interval_cmd,
       "min-lsa-interval <0-65535>",
       "minimum time between originating a particular LSA\n"
       "Seconds\n")
{
  struct ospf6 *o;

  o = (struct ospf6 *) vty->index;
  o->min_lsa_interval = strtol (argv[0], NULL, 10);

  return CMD_SUCCESS;
}

static int
ospf6_set_ipv6_protocol_traffic_class (struct vty *vty,
				       struct ospf6 *o, int tclass)
{
  int ret;

  if (tclass == o->ipv6_tclass)
    return CMD_SUCCESS;

  ret = ospf6_set_transport_class (tclass);
  if (ret)
    {
      vty_out (vty, "Setting the OSPFv3 traffic class to %d failed: %s%s",
	       tclass, safe_strerror (errno), VNL);
      return CMD_WARNING;
    }

  o->ipv6_tclass = tclass;

  return CMD_SUCCESS;
}

DEFUN (ospf6_ipv6_protocol_traffic_class,
       ospf6_ipv6_protocol_traffic_class_cmd,
       "protocol-traffic-class <0-255>",
       "The traffic class used for outgoing protocol packets\n"
       "Traffic class value\n")
{
  struct ospf6 *o = (struct ospf6 *) vty->index;
  int tclass;

  tclass = strtol (argv[0], NULL, 10);

  return ospf6_set_ipv6_protocol_traffic_class (vty, o, tclass);
}

DEFUN (no_ospf6_ipv6_protocol_traffic_class,
       no_ospf6_ipv6_protocol_traffic_class_cmd,
       "no protocol-traffic-class",
       NO_STR
       "Use the default traffic class for outgoing protocol packets\n")
{
  ...
  tclass = OSPF6_DEFAULT_IPV6_TCLASS;

  return ospf6_set_ipv6_protocol_traffic_class (vty, o, tclass);
}

static void
ospf6_af_set_interoperability (struct ospf6 *o, bool enable)
{
  bool af_active;

  if ((o->af_interop && enable) || (!o->af_interop && !enable))
    return;

  if (ospf6_af_is_ipv4 (o) && !CHECK_FLAG (o->flag, OSPF6_DISABLED))
    af_active = true;
  else
    af_active = false;
...
DEFUN (ospf6_af_interoperability,
       ospf6_af_interoperability_cmd,
       "af-interoperability",
       "RFC 5838 Address Families interoperability\n")
{
  struct ospf6 *o;

  o = (struct ospf6 *) vty->index;

  ospf6_af_set_interoperability (o, true);

  return CMD_SUCCESS;
}

DEFUN (no_ospf6_af_interoperability,
       no_ospf6_af_interoperability_cmd,
       ...
       "RFC 5838 Address Families interoperability\n")
{
  ...
  ospf6_af_set_interoperability (o, false);

  return CMD_SUCCESS;
}

DEFUN (ospf6_mdr_tlv_interoperability,
       ospf6_mdr_tlv_interoperability_cmd,
       "mdr-tlv-interoperability",
       "RFC 5614 MDR TLV interoperability\n")
{
  struct ospf6 *o;

  o = (struct ospf6 *) vty->index;

  ospf6_mdr_tlv_set_interoperability (o, true);

  return CMD_SUCCESS;
}

DEFUN (no_ospf6_mdr_tlv_interoperability,
       no_ospf6_mdr_tlv_interoperability_cmd,
       ...
       "RFC 5614 MDR TLV interoperability\n")
{
  ...
  ospf6_mdr_tlv_set_interoperability (o, false);
...
ospf6_show (struct vty *vty, struct ospf6 *o)
{
  ...
  if (o->router_id == 0)
    {
      vty_out (vty, " OSPFv3 is not enabled%s", VNL);
      return;
    }

  /* process id, router id */
  ospf6_id2str (o->router_id, router_id, sizeof (router_id));
  vty_out (vty, " OSPFv3 Routing Process (0), Instance ID %u, Router-ID %s%s",
           o->instance_id, router_id, VNL);
...
ALIAS (show_ipv6_ospf6_route,
       ...
       "show ipv6 ospf6 route (X:X::X:X|X:X::X:X/M|A.B.C.D|A.B.C.D/M|detail|summary)",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       ROUTE_STR
...
DEFUN (show_ipv6_ospf6_route_match,
       ...
       "show ipv6 ospf6 route (X:X::X:X/M|A.B.C.D/M) match",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       ROUTE_STR
...
DEFUN (show_ipv6_ospf6_route_match_detail,
       ...
       "show ipv6 ospf6 route (X:X::X:X/M|A.B.C.D/M) match detail",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       ROUTE_STR
...
ALIAS (show_ipv6_ospf6_route_match,
       ...
       "show ipv6 ospf6 route (X:X::X:X/M|A.B.C.D/M) longer",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       ROUTE_STR
...
DEFUN (show_ipv6_ospf6_route_match_detail,
       ...
       "show ipv6 ospf6 route (X:X::X:X/M|A.B.C.D/M) longer detail",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       ROUTE_STR
...
config_write_ospf6 (struct vty *vty)
{
  ...
  ospf6_id2str (ospf6->router_id_static, router_id, sizeof (router_id));
  vty_out (vty, "router ospf6%s", VNL);
  if (ospf6->ipv6_tclass != OSPF6_DEFAULT_IPV6_TCLASS)
    vty_out (vty, " protocol-traffic-class %d%s", ospf6->ipv6_tclass, VNL);
  if (!ospf6->af_interop)
    vty_out (vty, " no af-interoperability%s", VNL);
  if (!ospf6->mdr_tlv_interop)
    vty_out (vty, " no mdr-tlv-interoperability%s", VNL);
  ...
  if (ospf6->min_lsa_interval != MIN_LS_INTERVAL)
    vty_out (vty, " min-lsa-interval %u%s", ospf6->min_lsa_interval, VNL);
...
ospf6_top_init (void)
{
  ...
  install_element (OSPF6_NODE, &ospf6_instance_id_cmd);
  install_element (OSPF6_NODE, &ospf6_interface_area_cmd);
  install_element (OSPF6_NODE, &no_ospf6_interface_area_cmd);
  install_element (OSPF6_NODE, &ospf6_min_lsa_arrival_cmd);
  install_element (OSPF6_NODE, &ospf6_min_lsa_interval_cmd);
  install_element (OSPF6_NODE, &ospf6_ipv6_protocol_traffic_class_cmd);
  install_element (OSPF6_NODE, &no_ospf6_ipv6_protocol_traffic_class_cmd);
  install_element (OSPF6_NODE, &ospf6_af_interoperability_cmd);
