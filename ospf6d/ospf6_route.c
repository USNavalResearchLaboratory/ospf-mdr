...
u_int32_t
ospf6_adv_router_in_prefix (struct prefix *prefix)
{
  u_int32_t *adv_router;

  adv_router = (u_int32_t *)&prefix->u.prefix6.s6_addr[0];

  return *adv_router;
}

u_int32_t
ospf6_linkstate_prefix_adv_router (struct prefix *prefix)
{
  return ospf6_adv_router_in_prefix (prefix);
}

u_int32_t
ospf6_linkstate_prefix_id (struct prefix *prefix)
{
  u_int32_t *id;

  id = (u_int32_t *)&prefix->u.prefix6.s6_addr[4];
...
ospf6_linkstate_prefix2str (struct prefix *prefix, char *buf, int size)
{
  ...
  ospf6_id2str (adv_router, adv_router_str, sizeof (adv_router_str));
  ospf6_id2str (id, id_str, sizeof (id_str));
  if (ntohl (id))
...
ospf6_route_copy (struct ospf6_route *route)
{
  ...
  new->type = route->type;
  new->prefix = route->prefix;
  new->path = route->path;
  for (i = 0; i < OSPF6_MULTI_PATH_LIMIT; i++)
    new->nexthop[i] = route->nexthop[i];
  ...
  if (prev || next)
    {
      ...
      if (node->info == next)
	    ...
	    zlog_info ("%s: route add: replacing previous best",
		       ospf6_route_table_name (table));
        }
                    ...
                    struct ospf6_route_table *table)
{
  ...
  assert (!CHECK_FLAG (route->flag, OSPF6_ROUTE_WAS_REMOVED));
  ...
  if (node->info == route)
    {
	  ...
	  if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
	    zlog_info ("%s %p: route remove %p: new best: %p",
		       ospf6_route_table_name (table), table,
		       route, route->next);
	  else if (IS_OSPF6_DEBUG_ROUTE (TABLE))
	    zlog_info ("%s: route remove: new best",
		       ospf6_route_table_name (table));
        }
      else
...
ospf6_route_show (struct vty *vty, struct ospf6_route *route)
{
  ...
  if (route->type == OSPF6_DEST_TYPE_LINKSTATE)
    ...
    ospf6_prefix2str (ospf6, &route->prefix, destination, sizeof (destination));
...
ospf6_route_show_detail (struct vty *vty, struct ospf6_route *route)
{
  ...
  char area_id[16], id[16], adv_router[16], capa[16], options[OSPF6OPTSTRLEN];
  struct timeval now, res;
  char duration[16];
  int i;
  ...
  /* Area-ID */
  ospf6_id2str (route->path.area_id, area_id, sizeof (area_id));
  ...
  /* LS Origin */
  ospf6_id2str (route->path.origin.id, id, sizeof (id));
  ospf6_id2str (route->path.origin.adv_router, adv_router,
		sizeof (adv_router));
  ...
  for (i = 0; i < argc; i++)
    {
      ...
      ret = ospf6_str2prefix (ospf6, argv[i], &prefix);
      if (ret == 1)
        {
          isprefix++;
          if (strchr (argv[i], '/'))
...
{
  u_int32_t router, id;
  char routername[16], idname[16], rbits[16], options[OSPF6OPTSTRLEN];

  router = ospf6_linkstate_prefix_adv_router (&route->prefix);
  ospf6_id2str (router, routername, sizeof (routername));
  id = ospf6_linkstate_prefix_id (&route->prefix);
  ospf6_id2str (id, idname, sizeof (idname));
...
{
  u_int32_t adv_router;
  char adv[16], rbits[16], options[OSPF6OPTSTRLEN], area[16];

  adv_router = ospf6_linkstate_prefix_adv_router (&route->prefix);
  ospf6_id2str (adv_router, adv, sizeof (adv));
  ospf6_capability_printbuf (route->path.router_bits, rbits, sizeof (rbits));
  ospf6_options_printbuf (route->path.options, options, sizeof (options));
  ospf6_id2str (route->path.area_id, area, sizeof (area));
