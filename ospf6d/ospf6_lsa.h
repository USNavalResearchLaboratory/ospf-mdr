...
#define OSPF6_LSA_DEBUG_MASK      0x0f
...
struct ospf6_lsa
{
  ...
  struct thread *backupWaitTimer;
  struct list *backupwait_neighbor_list;
};
...
struct ospf6_lsa_handler
{
  ...
  u_char flags;	   /* lower 4 bits used for OSPF6_LSA_DEBUG options */
#define OSPF6_LSA_HANDLER_DYNAMIC 0x10
};
...
#define CONTINUE_IF_V4_ADDRESS_LOOPBACK(debug,addr)          \
  if (((ntohl((long int) (addr->u.prefix4.s_addr))) & 0xff000000)==0x7f000000) \
    {                                                        \
      char buf[64];                                          \
      prefix2str (addr, buf, sizeof (buf));                  \
      if (debug)                                             \
...
extern void ospf6_uninstall_lsa_handler (struct ospf6_lsa_handler *handler);
