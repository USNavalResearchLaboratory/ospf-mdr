...
int
ospf6_set_transport_class (int tclass)
{
  return setsockopt_ipv6_tclass (ospf6_sock, tclass);
}
...
int
ospf6_set_multicast_hops (int hoplimit)
{
  int err;

  err = setsockopt (ospf6_sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
		    &hoplimit, sizeof (hoplimit));
  if (err)
    zlog_warn ("Network: set IPV6_MULTICAST_HOPS failed for %d: %s",
	       hoplimit, safe_strerror (errno));
...
ospf6_serv_sock (void)
{
  ...
  ospf6_set_pktinfo ();
  ospf6_set_transport_class (OSPF6_DEFAULT_IPV6_TCLASS);
  ospf6_set_checksum ();
  ospf6_set_multicast_hops (1);
