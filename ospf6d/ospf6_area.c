...
static struct list ospf6_area_operations_list;
static unsigned int ospf6_area_init_called;
...
static void
ospf6_area_lsdb_hook_replace (struct ospf6_lsa *old, struct ospf6_lsa *new)
{
  u_int16_t type;

  assert (OSPF6_LSA_IS_SAME (old, new));

  type = ntohs (new->header->type);

  if (type == OSPF6_LSTYPE_INTRA_PREFIX)
    {
      ospf6_intra_prefix_lsa_replace (old, new);
    }
  else
    {
      ospf6_area_lsdb_hook_remove (old);
      ospf6_area_lsdb_hook_add (new);
    }
}
...
ospf6_area_create (u_int32_t area_id, struct ospf6 *o)
{
  ...
  struct ospf6_area_operations *ops;

  oa = XCALLOC (MTYPE_OSPF6_AREA, sizeof (struct ospf6_area));

  ospf6_id2str (area_id, oa->name, sizeof (oa->name));
  oa->area_id = area_id;
  oa->if_list = list_new ();
  ...
  oa->lsdb->hook_replace = ospf6_area_lsdb_hook_replace;
   ...
   */
  OSPF6_OPT_SET (oa->options, OSPF6_OPT_AF, 1);

  oa->spf_delay_msec = OSPF6_DEFAULT_SPF_DELAY_MSEC;
  oa->spf_holdtime_msec = OSPF6_DEFAULT_SPF_HOLDTIME_MSEC;

  oa->ospf6 = o;
  listnode_add_sort (o->area_list, oa);

  oa->private_data_list = ospf6_private_data_list ();

  for (ALL_LIST_ELEMENTS_RO (&ospf6_area_operations_list, node, ops))
    {
      int err;
	...
	continue;
      ...
      if (err)
	{
	  zlog_err ("%s: per area create function %p failed for area %s",
		    __func__, ops->create, oa->name);
...
ospf6_area_delete (struct ospf6_area *oa)
{
  ...
  if (!list_isempty (oa->private_data_list))
    zlog_err ("%s: possible memory leak: deleting oa->private_data_list "
	      "with %i elements", __func__, listcount (oa->private_data_list));
  list_delete (oa->private_data_list);
...
/**
 * Register area operations
 *
 * Area operations are a set of callbacks that apply to all ospf
 * areas.
 *
 * @param ops A pointer to the area operations structure to
...
{
  struct listnode *node;
  struct ospf6_area_operations *tmpops;

  for (ALL_LIST_ELEMENTS_RO (&ospf6_area_operations_list, node, tmpops))
    {
      if (tmpops == ops)
	{
	  zlog_err ("%s: per area operations already registered: %p",
		    __func__, ops);
	  return -1;
	}
    }

  listnode_add (&ospf6_area_operations_list, ops);

  if (ospf6_area_init_called && ops->init)
    ops->init ();
  ...
  if (ops->create && ospf6)
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
	{
	  ...
	  err = ops->create (oa);
	  if (err)
	    zlog_warn ("%s: per area create function %p failed for area %s",
		       __func__, ops->create, oa->name);
	}
    }
...
/**
 * Add private data to an ospf area
 *
 * Private data can be associated with an ospf area by calling
 * this function.  A unique data identifier, meant to be stable across
 * all areas, is used to refer to the data.  A new identifier is
 ...
 * which must not already be in use for the given area.
 *
 ...
 */
int
ospf6_area_add_data (struct ospf6_area *oa, unsigned int *id, void *data)
{
  return ospf6_add_private_data (oa->private_data_list, id, data);
}

/**
 * Get private data associated with an ospf area
 *
 * The given data identifier should be from an earlier
 * ospf6_area_add_data() call and must be nonzero.
 *
 * @param oa The ospf area.
 *
 ...
 */
void *
ospf6_area_get_data (struct ospf6_area *oa, unsigned int id)
{
  return ospf6_get_private_data (oa->private_data_list, id);
}

/**
 * Get and remove private data associated with an ospf area
 *
 * The given data identifier should be from an earlier
 * ospf6_area_add_data() call and must be nonzero.
 *
 ...
 */
void *
ospf6_area_del_data (struct ospf6_area *oa, unsigned int id)
{
  return ospf6_del_private_data (oa->private_data_list, id);
}
...
ospf6_area_disable (struct ospf6_area *oa)
{
  ...
  ospf6_route_remove_all (oa->range_table);
  ospf6_route_remove_all (oa->summary_prefix);
  ospf6_route_remove_all (oa->summary_router);

  ospf6_lsdb_remove_all (oa->lsdb);
  ospf6_lsdb_remove_all (oa->lsdb_self);

  ospf6_spf_table_finish (oa->spf_table);
  ospf6_route_remove_all (oa->spf_table);
...
DEFUN (area_range,
       area_range_cmd,
       "area (A.B.C.D|<0-4294967295>) range (X:X::X:X/M|A.B.C.D/M)",
       ...
       )
{
  ...
  ret = ospf6_str2prefix (oa->ospf6, argv[0], &prefix);
  if (ret == 0)
    {
...
ALIAS (area_range,
       ...
       "area (A.B.C.D|<0-4294967295>) range (X:X::X:X/M|A.B.C.D/M) (advertise|not-advertise)",
       "OSPF area parameters\n"
       OSPF6_AREAID_DOT_STR
       OSPF6_AREAID_VAL_STR
       "Configured address range\n"
...
DEFUN (no_area_range,
       no_area_range_cmd,
       "no area (A.B.C.D|<0-4294967295>) range (X:X::X:X/M|A.B.C.D/M)",
       NO_STR
       "OSPF area parameters\n"
       OSPF6_AREAID_DOT_STR
       OSPF6_AREAID_VAL_STR
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      ...
      for (range = ospf6_route_head (oa->range_table); range;
        ...
        {
          ospf6_prefix2str (oa->ospf6, &range->prefix, buf, sizeof (buf));
      ...
      if (oa->spf_delay_msec != OSPF6_DEFAULT_SPF_DELAY_MSEC)
	vty_out (vty, " area %s spf-delay-msec %u%s",
		 oa->name, oa->spf_delay_msec, VNL);
      if (oa->spf_holdtime_msec != OSPF6_DEFAULT_SPF_HOLDTIME_MSEC)
	vty_out (vty, " area %s spf-holdtime-msec %u%s",
		 oa->name, oa->spf_holdtime_msec, VNL);

      for (ALL_LIST_ELEMENTS_RO (&ospf6_area_operations_list, node, ops))
	if (ops && ops->config_write)
	  ops->config_write (oa, vty);
    }
}
...
DEFUN (area_spf_delay_msec,
       area_spf_delay_msec_cmd,
       "area (A.B.C.D|<0-4294967295>) spf-delay-msec <0-10000>",
       "OSPFv6 area parameters\n"
       OSPF6_AREAID_DOT_STR
       OSPF6_AREAID_VAL_STR
       "Time to delay SPF calculations\n"
       "Milliseconds\n")
{
  struct ospf6_area *oa;
  ...
  oa->spf_delay_msec = strtol (argv[1], NULL, 10);

  return CMD_SUCCESS;
}

DEFUN (area_spf_holdtime_msec,
       area_spf_holdtime_msec_cmd,
       "area (A.B.C.D|<0-4294967295>) spf-holdtime-msec <0-10000>",
       "OSPFv6 area parameters\n"
       OSPF6_AREAID_DOT_STR
       OSPF6_AREAID_VAL_STR
       "Minimum time between SPF calculations\n"
       "Milliseconds\n")
{
  struct ospf6_area *oa;
  ...
  oa->spf_holdtime_msec = strtol (argv[1], NULL, 10);
       ...
       )
{
  ...
  OSPF6_CMD_AREA_LOOKUP (argv[1], oa);
...
ospf6_area_init (void)
{
  ...
  install_element (OSPF6_NODE, &area_spf_delay_msec_cmd);
  install_element (OSPF6_NODE, &area_spf_holdtime_msec_cmd);

  for (ALL_LIST_ELEMENTS_RO (&ospf6_area_operations_list, node, ops))
    if (ops && ops->init)
      ops->init ();

  ospf6_area_init_called++;
}

void
ospf6_area_terminate (void)
{
  list_delete_all_node (&ospf6_area_operations_list);
}
