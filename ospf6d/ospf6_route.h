...
#define ospf6_nexthop_clear(x)                                \
  do {                                                        \
    memset (x, 0, sizeof (struct ospf6_nexthop));             \
  } while (0)
...
#define ospf6_route_is_same(ra, rb) \
  (prefix_cmp (&(ra)->prefix, &(rb)->prefix) == 0)
...
extern void ospf6_linkstate_prefix2str (struct prefix *prefix, char *buf,
                                        int size);
extern u_int32_t ospf6_adv_router_in_prefix (struct prefix *prefix);
extern u_int32_t ospf6_linkstate_prefix_adv_router (struct prefix *prefix);
extern u_int32_t ospf6_linkstate_prefix_id (struct prefix *prefix);
