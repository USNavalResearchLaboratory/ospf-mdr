      ...
      )
{
  ...
  if (ospf6_str2id (argv[0], &adv_router))
    {
...
static int
ospf6_lsdesc_exists (struct ospf6_lsa *lsa, u_int32_t router_id,
		     u_int32_t interface_id, u_int32_t nbr_interface_id,
		     u_int8_t type)
{
  struct ospf6_router_lsdesc *lsdesc, *lsaend;

  assert (OSPF6_LSA_IS_TYPE (ROUTER, lsa));

  lsaend = (struct ospf6_router_lsdesc *) OSPF6_LSA_END (lsa->header);

  for (lsdesc =
	 (struct ospf6_router_lsdesc *) (OSPF6_LSA_HEADER_END (lsa->header) +
					 sizeof (struct ospf6_router_lsa));
       lsdesc + 1 <= lsaend; lsdesc++)
    {
      if (lsdesc->type == type &&
	  lsdesc->interface_id == nbr_interface_id &&
	  lsdesc->neighbor_interface_id == interface_id &&
	  lsdesc->neighbor_router_id == router_id)
	return 1;
    }
...
static void
ospf6_database_asymmetric_check (struct ospf6_lsdb *lsdb,
				 void (*asymmetric_callback) (u_int32_t router_id,
							      u_int32_t interface_id,
							      u_int32_t neighbor_router_id,
							      u_int32_t neighbor_interface_id,
							      u_int8_t type,
							      void *data),
				 void *asymmetric_callback_data)
{
  u_int16_t lstype_router = htons (OSPF6_LSTYPE_ROUTER);
  struct ospf6_lsa *lsa;

  for (lsa = ospf6_lsdb_type_head (lstype_router, lsdb);
       lsa != NULL;
       lsa = ospf6_lsdb_type_next (lstype_router, lsa))
    {
	...
	*lsaend = (struct ospf6_router_lsdesc *) OSPF6_LSA_END (lsa->header);
      ...
      for (lsdesc =
	   ...
	   lsdesc + 1 <= lsaend; lsdesc++)
	{
	  u_char type = lsdesc->type;
	  u_int32_t interface_id = lsdesc->interface_id;
	  u_int32_t neighbor_router_id = lsdesc->neighbor_router_id;
	  u_int32_t neighbor_interface_id = lsdesc->neighbor_interface_id;
	  struct ospf6_lsa *nbrlsa;
	  int symmetric;

	  nbrlsa = ospf6_lsdb_lookup (lstype_router, 0,
				      neighbor_router_id, lsdb);
	  if (nbrlsa)
	    symmetric = ospf6_lsdesc_exists (nbrlsa, adv_router, interface_id,
					     neighbor_interface_id, type);
	  else
	    symmetric = 0;

	  if (!symmetric)
	    asymmetric_callback (adv_router, interface_id,
				 neighbor_router_id, neighbor_interface_id,
				 type, asymmetric_callback_data);
	}
    }
}

static void
ospf6_database_asymmetric_callback_vtyout (u_int32_t router_id,
					   u_int32_t interface_id,
					   u_int32_t neighbor_router_id,
					   u_int32_t neighbor_interface_id,
					   u_int8_t type, void *callback_data)
{
  struct vty *vty = callback_data;
  char rid[INET_ADDRSTRLEN], lsid[INET_ADDRSTRLEN],
    nbr_rid[INET_ADDRSTRLEN], nbr_lsid[INET_ADDRSTRLEN];

  ospf6_id2str (router_id, rid, sizeof (rid));
  ospf6_id2str (interface_id, lsid, sizeof (lsid));
  ospf6_id2str (neighbor_router_id, nbr_rid, sizeof (nbr_rid));
  ospf6_id2str (neighbor_interface_id, nbr_lsid, sizeof (nbr_lsid));

  vty_out (vty, "asymmetric link (type %u): %s [%s] -> %s [%s]%s",
	   type, rid, lsid, nbr_rid, nbr_lsid, VNL);
}

DEFUN (show_ipv6_ospf6_database_asymmetric_links,
       show_ipv6_ospf6_database_asymmetric_links_cmd,
       "show ipv6 ospf6 database asymmetric-links",
       SHOW_STR
       IP6_STR
       OSPF6_STR
       "Display Link state database\n"
       "Display Asymmetric Links\n"
       )
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    ospf6_database_asymmetric_check (oa->lsdb,
				     ospf6_database_asymmetric_callback_vtyout,
				     vty);
...
#define INSTALL(n,c) \
  ...
  INSTALL (VIEW, database_asymmetric_links_cmd);
