...
zclient_start (struct zclient *zclient)
{
  ...
  if (zclient->linkmetrics_subscribe)
    zclient_send_linkmetrics_subscribe (zclient, ZEBRA_LINKMETRICS_SUBSCRIBE);
...
int
zclient_linkmetrics_subscribe (struct zclient *zclient, uint16_t cmd)
{
  switch (cmd)
    {
    case ZEBRA_LINKMETRICS_SUBSCRIBE:
      if (zclient->linkmetrics_subscribe)
	return 0;
      zclient->linkmetrics_subscribe = 1;
      break;

    case ZEBRA_LINKMETRICS_UNSUBSCRIBE:
      if (!zclient->linkmetrics_subscribe)
	return 0;
      zclient->linkmetrics_subscribe = 0;
      break;

    default:
      zlog_err ("%s: unknown zebra linkmetrics subscribe command: %u",
		__func__, cmd);
      return -1;
      break;
    }

  return zclient_send_linkmetrics_subscribe (zclient, cmd);
}
