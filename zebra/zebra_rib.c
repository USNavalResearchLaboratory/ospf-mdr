...

/**
 * Control if nexthops are sorted
 *
 * Nexthops are added to rib entries in order when nonzero.
 * Descending order is used when @p sort_nexthops > 0; ascending order
 * when @p sort_nexthops < 0.
 */
static int sort_nexthops = 0;

static void
set_sort_nexthops (int val)
{
  if (sort_nexthops == val)
    return;

  /* TODO: sort existing rib entries */

  sort_nexthops = val;
}

DEFUN (rib_sort_nexthops_descending,
       rib_sort_nexthops_descending_cmd,
       "rib sort-nexthops descending",
       "RIB information\n"
       "Sort RIB entry next hops\n"
       "Descending order\n")
{
  set_sort_nexthops (1);
...
DEFUN (rib_sort_nexthops_ascending,
       rib_sort_nexthops_ascending_cmd,
...
DEFUN (no_rib_sort_nexthops,
       no_rib_sort_nexthops_cmd,
...
/* compare nexthops by ifindex */
static int
nexthop_cmp_ifindex (struct nexthop *a, struct nexthop *b)
{
...
/* compare nexthops by ifname */
static int
nexthop_cmp_ifname (struct nexthop *a, struct nexthop *b)
{
  return strcmp (a->ifname, b->ifname);
}

/* compare nexthops by ipv4 address */
static int
nexthop_cmp_ipv4 (struct nexthop *a, struct nexthop *b)
{
  u_int32_t _a, _b;

  _a = ntohl (a->gate.ipv4.s_addr);
  _b = ntohl (b->gate.ipv4.s_addr);
...
/* compare nexthops by ipv6 address */
static int
nexthop_cmp_ipv6 (struct nexthop *a, struct nexthop *b)
{
  return memcmp (&a->gate.ipv6, &b->gate.ipv6, sizeof (struct in6_addr));
}

/**
 * Nexthop comparison helper function.
 *
 * Compare nexthops when at least one is a type at the current
 * priority level, meaning @p a_is_type or @p b_is_type is nonzero.
 *
 * @param a The first nexthop
 *
 * @param b The second nexthop
 *
 * @param a_is_type Nonzero when the nexthop type of @p a is at the
 *                  current priority level
 *
 * @param b_is_type Nonzero when the nexthop type of @p b is at the
 *                  current priority level
 *
 * @param compar Comparison function to use when @p a_is_type and @p
 *               b_is_type are both nonzero
 *
 * @param result Pointer to where the result is stored when @p a and
 *               @p b can be compared

 * @return Nonzero when @p a and @p p were sucessfully compared; zero
 *         otherwise
 */
static int
nexthop_cmp_type (struct nexthop *a, struct nexthop *b,
		  int a_is_type, int b_is_type,
		  int (*compar) (struct nexthop *, struct nexthop *),
		  int *result)
{
...
/**
 * Compare two nexthops.
 *
 * The type of each nexthop is considered first where:
 *
 *  NEXTHOP_TYPE_IFINDEX >
 *  NEXTHOP_TYPE_IFNAME >
 *  NEXTHOP_TYPE_IPV4 | NEXTHOP_TYPE_IPV4_IFINDEX | NEXTHOP_TYPE_IPV4_IFNAME >
 *  NEXTHOP_TYPE_IPV6 | NEXTHOP_TYPE_IPV6_IFINDEX | NEXTHOP_TYPE_IPV6_IFNAME >
 *  NEXTHOP_TYPE_BLACKHOLE
 *
 * When the type of both nexthops is NEXTHOP_TYPE_IFINDEX, ifindex
 * values are compared.
 *
 * When the type of both nexthops is NEXTHOP_TYPE_IFNAME, ifname
 * strings are compared.
 *
 * When both nexthops include IPv4 information or both nexthops
 * include IPv6 information, nexthop addresses are compared.
 *
 * NEXTHOP_TYPE_BLACKHOLE nexthops are equal.
 *
 * @param a The first nexthop
 * @param b The second nexthop
 *
 ...
 */
static int
nexthop_cmp (struct nexthop *a, struct nexthop *b)
{
  int a_is_type, b_is_type, cmp, done;

  a_is_type = (a->type == NEXTHOP_TYPE_IFINDEX);
  b_is_type = (b->type == NEXTHOP_TYPE_IFINDEX);
  done = nexthop_cmp_type (a, b, a_is_type, b_is_type,
			   nexthop_cmp_ifindex, &cmp);
  if (done)
    return cmp;

  a_is_type = (a->type == NEXTHOP_TYPE_IFNAME);
  b_is_type = (b->type == NEXTHOP_TYPE_IFNAME);
  done = nexthop_cmp_type (a, b, a_is_type, b_is_type,
			   nexthop_cmp_ifname, &cmp);
  if (done)
    return cmp;

#define NEXTHOP_IS_IPV4(x)			\
  ((x)->type == NEXTHOP_TYPE_IPV4 ||		\
   (x)->type == NEXTHOP_TYPE_IPV4_IFINDEX ||	\
   (x)->type == NEXTHOP_TYPE_IPV4_IFNAME)

  a_is_type = NEXTHOP_IS_IPV4 (a);
  b_is_type = NEXTHOP_IS_IPV4 (b);
  done = nexthop_cmp_type (a, b, a_is_type, b_is_type,
			   nexthop_cmp_ipv4, &cmp);
  if (done)
    return cmp;

#undef NEXTHOP_IS_IPV4

#define NEXTHOP_IS_IPV6(x)			\
  ((x)->type == NEXTHOP_TYPE_IPV6 ||		\
   (x)->type == NEXTHOP_TYPE_IPV6_IFINDEX ||	\
   (x)->type == NEXTHOP_TYPE_IPV6_IFNAME)

  a_is_type = NEXTHOP_IS_IPV6 (a);
  b_is_type = NEXTHOP_IS_IPV6 (b);
  done = nexthop_cmp_type (a, b, a_is_type, b_is_type,
			   nexthop_cmp_ipv6, &cmp);
  if (done)
    return cmp;

#undef NEXTHOP_IS_IPV6

  assert (a->type == NEXTHOP_TYPE_BLACKHOLE &&
	  b->type == NEXTHOP_TYPE_BLACKHOLE);

  return 0;
}

/* Add nexthop to the list. */
static void
nexthop_add (struct rib *rib, struct nexthop *nexthop)
{
  struct nexthop *predecessor;

  for (predecessor = rib->nexthop; predecessor;
      ...
      if (sort_nexthops)
	{
	  ...
	  cmp = nexthop_cmp (nexthop, predecessor);
	  if (sort_nexthops < 0)
	    cmp = -cmp;
	  if (cmp > 0)
	    {
  ...
  nexthop->prev = predecessor;

  if (predecessor)
    {
      nexthop->next = predecessor->next;
      if (predecessor->next)
	predecessor->next->prev = nexthop;
      predecessor->next = nexthop;
    }
  else
    {
      nexthop->next = rib->nexthop;
      if (rib->nexthop)
	rib->nexthop->prev = nexthop;
      rib->nexthop = nexthop;
    }
...
rib_uninstall_kernel (struct route_node *rn, struct rib *rib)
{
  ...
  switch (PREFIX_FAMILY (&rn->p))
    ...
    case AF_INET6:
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s: calling kernel_delete_ipv6 (%p, %p)", __func__, rn, rib);
...
rib_process (struct route_node *rn)
{
  ...
  for (rib = rn->info; rib; rib = next)
	...
	{
	  if (IS_ZEBRA_DEBUG_RIB)
	    zlog_debug ("%s: skipping %s/%d because nexthop is not active",
...

static int
config_write_rib (struct vty *vty)
{
  if (sort_nexthops > 0)
    vty_out (vty, "rib sort-nexthops descending%s", VTY_NEWLINE);
  else if (sort_nexthops < 0)
    vty_out (vty, "rib sort-nexthops ascending%s", VTY_NEWLINE);

  return 0;
}

/* RIB node */
static struct cmd_node rib_node =
{
  RIB_NODE,
...
rib_init (void)
{
  ...
  install_node (&rib_node, config_write_rib);

  install_element (CONFIG_NODE, &rib_sort_nexthops_descending_cmd);
  install_element (CONFIG_NODE, &rib_sort_nexthops_ascending_cmd);
  install_element (CONFIG_NODE, &no_rib_sort_nexthops_cmd);
}
